...
class MIPV1DeviceDidAddBridgedDevice: MIPV1DeviceNotification {

    var type : MIPV1DeviceNotificationType { return .didAddBridgedDevice }

    let bridgedDevice: DeviceProfile

    private enum CodingKeys: CodingKey {
        case bridgedDevice
    }

    init(_ bridgedDevice: DeviceProfile)
    {
    ...
    required init(from decoder: Decoder) throws
    {
        ...
        bridgedDevice = try container.decode(DeviceProfile.self, forKey: .bridgedDevice)
    }
    ...
    {
        client.device(device, didAddBridgedDevice: bridgedDevice)
    }
